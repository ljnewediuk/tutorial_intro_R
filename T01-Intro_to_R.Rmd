...
When you create a new file, it will appear as a new panel in the upper-left of your RStudio console. You can save the new file by clicking on the "Save" icon in the upper-left corner of the new file---save often! You will need to submit saved files to complete your takehome challenges and project.
...
Everyone will need to use packages in this course. To install the package, write the following code in your console and run it:

```{r, eval = F}
install.packages('rmarkdown', dependencies = T)
```

The first package we will install is called *"rmarkdown"*. You will need this package to read, edit, and preview your *R Markdown* files. R Markdown is another interface to R that "renders" text and code you've written into familiar file types like MS Word documents, PDF files, and even webpages. For example, the webpage you are reading now was rendered using R Markdown.

You will see a long read-out of information in the console as the package installs. Once installed, you can "attach" the package by typing and running this code:

```{r, eval = F}
library(rmarkdown)
```

Again, you will see a shorter read-out as the package loads. Many students make the mistake of installing packages without attaching them. In order to use the package's functions, you must first attach it.

## 4 - Scripts versus R Markdown files

### R Markdown

R Markdown files you receive will include a combination of code and plain text. Hash tags tell R Markdown to create a heading. Asterisks are used to make text bold and italic. Special regions called *"code chunks"* allow you to write and execute your code within the rendered document---you will edit these chunks to complete your assignment. Depending on the code chunk, you will see the code itself, the result of executing the code (e.g., a graph), or both. This is an example of what you might see after rendering an R Markdown file:
...
In this case, the result is an MS Word document. You can see some headings and a table with some data. The table was printed using a code chunk. 

As you work on your R Markdown, you can preview your progress by *"knitting"* it. We are going to knit our most of our documents in MS Word. To knit your document, we will need to install a new package called *"knitr"*:

```{r, eval = F}
install.packages('knitr', dependencies = T)
```

After you have installed knitr, load the package. Then click the down-arrow beside the knit button, which looks like a ball of yarn with a knitting needle, at the top of your file. In the drop-down menu, select "Knit to Word". A Word document will open, displaying your rendered document. When you are happy with your R Markdown document, you can save the word document.

```{r echo = FALSE, fig.cap = "<center>*Knitting an Rmarkdown file*</center>"}
knitr::include_graphics("imgs/rmarkdown_render.png")
```

You can also run code using the arrows at the top right of the code chunk. The grey downward arrow runs everything previous up to the current code chunk. The green arrow pointing to your right runs the code within the current code chunk only.
